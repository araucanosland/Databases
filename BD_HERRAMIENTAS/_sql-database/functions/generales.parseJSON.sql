IF EXISTS (SELECT 1 FROM sys.objects WHERE object_id = OBJECT_ID('[generales].[parseJSON]') AND type = 'TF')
DROP FUNCTION [generales].[parseJSON]
GO
CREATE FUNCTION [generales].[parseJSON]( @JSON NVARCHAR(MAX))
