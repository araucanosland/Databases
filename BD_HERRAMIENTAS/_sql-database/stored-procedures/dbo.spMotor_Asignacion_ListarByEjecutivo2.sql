IF EXISTS (SELECT 1 FROM sys.objects WHERE object_id = OBJECT_ID('[dbo].[spMotor_Asignacion_ListarByEjecutivo2]') AND type = 'P')
DROP PROCEDURE [dbo].[spMotor_Asignacion_ListarByEjecutivo2]
GO
CREATE PROCEDURE [dbo].[spMotor_Asignacion_ListarByEjecutivo2]
	@Periodo AS int ,
...
/*	union
	...
	select DISTINCT a.*, c.* 
	from TabMotor_Asignacion a with(index(idx_ListarByOficina2))
	inner join TabMotor_Gestion b  with(index(IDX_MATCH_ASG))
	ON a.id_Asign = b.ges_bcam_uid and b.ges_ejecutivo_rut = @RutEjec
	outer apply(
			select * 
			from dbo.TabMotor_Gestion xx  with(index(DX4))
			where ges_bcam_uid = a.id_Asign
