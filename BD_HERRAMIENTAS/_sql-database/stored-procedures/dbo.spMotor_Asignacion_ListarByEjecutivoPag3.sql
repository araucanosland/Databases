IF EXISTS (SELECT 1 FROM sys.objects WHERE object_id = OBJECT_ID('[dbo].[spMotor_Asignacion_ListarByEjecutivoPag3]') AND type = 'P')
DROP PROCEDURE [dbo].[spMotor_Asignacion_ListarByEjecutivoPag3]
GO
CREATE PROCEDURE [dbo].[spMotor_Asignacion_ListarByEjecutivoPag3]
	@Periodo int ,
	@TipoCamp int,
	@Estado int=0,
	@Marca varchar(50) = '',
  ...
  -- routine body goes here, e.g.
	...
	BEGIN
			...
			from (
							...
							and (@Marca <> '' and (@Marca = 'Pensionados' and (Antiguedad_en_Meses = 0 and (EsPensionado = 1 or UPPER(Segmento) = 'PENSIONADOS'))) or (@Marca = 'Cumpleanos' and   month(FechaNacimiento) =  month(CONVERT(date, SYSDATETIME())) and day(FechaNacimiento) =  day(CONVERT(date, SYSDATETIME())))or @Marca = '')
							and (@BusquedaGeneral <> '' and (Empresa = @BusquedaGeneral)or @BusquedaGeneral = '')
							--Antiguedad_en_Meses = 0 and EsPensionado = 1
							union
							select a.id_Asign, 
														a.Periodo, 
														a.Afiliado_Rut, 
			...
			SET @Query = '
				select * from (
						...
						from (
							...
							and (@Marca_ <> '''' and (@Marca_ = ''Pensionados'' and (Antiguedad_en_Meses = 0 and (EsPensionado = 1 or UPPER(Segmento) = ''PENSIONADOS''))) or (@Marca_ = ''Cumpleanos'' and   month(FechaNacimiento) =  month(CONVERT(date, SYSDATETIME())) and day(FechaNacimiento) =  day(CONVERT(date, SYSDATETIME())))or @Marca_ = '''')
							and (@BusquedaGeneral_ <> '''' and (Empresa = @BusquedaGeneral_ )or @BusquedaGeneral_ = '''')
							union
							select a.id_Asign, 
														a.Periodo, 
														a.Afiliado_Rut, 
			...
			EXECUTE sp_executesql	@Query, 
									N'@RutEjec_ varchar(20), @Periodo_ int, @SubEstado_ int, @Estado_ int, @Marca_ varchar(50), @Prioridad_ varchar(20), @Segmento_ varchar(20), @Tipo_ varchar(20), @RutAfiliado_ varchar(20), @BusquedaGeneral_ varchar(200), @TotalReg_ int, @Offset_ int, @Limit_ int, @Vencimiento_ varchar(50)',
									@RutEjec_ =  @RutEjec, @Periodo_ = @Periodo, @SubEstado_= @SubEstado, @Estado_ = @Estado, @Marca_ = @Marca, @Prioridad_ = @Prioridad, @Segmento_ = @Segmento, @Tipo_ = @Tipo, @RutAfiliado_ = @RutAfiliado, @BusquedaGeneral_ = @BusquedaGeneral ,@TotalReg_ = @TotalReg, @Offset_ = @Offset, @Limit_ = @Limit, @Vencimiento_ = @Vencimiento
