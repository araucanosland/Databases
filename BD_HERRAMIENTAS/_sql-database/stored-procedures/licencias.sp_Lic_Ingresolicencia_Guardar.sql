IF EXISTS (SELECT 1 FROM sys.objects WHERE object_id = OBJECT_ID('[licencias].[sp_Lic_Ingresolicencia_Guardar]') AND type = 'P')
DROP PROCEDURE [licencias].[sp_Lic_Ingresolicencia_Guardar]
GO


CREATE PROCEDURE [licencias].[sp_Lic_Ingresolicencia_Guardar]
(
	@CodIngreso bigint,
...
BEGIN
	...
	select @identity=CodIngreso,@EsMismaOfi=1 
		from BD_HERRAMIENTAS.licencias.TabLic_IngresoLicencia
	...
	begin
	UPDATE BD_HERRAMIENTAS.licencias.TabLic_IngresoLicencia SET 
	...
	BEGIN
		...
		BEGIN--55695593
				...
				INSERT INTO BD_HERRAMIENTAS.licencias.TabLic_IngresoLicencia
				(
					RutAfiliado,
					NombreAfiliado,
					FolioLicencia,
...
--	IF @identity > 0
--	BEGIN
--		UPDATE TabLic_IngresoLicencia SET 
--			RutAfiliado = @RutAfiliado,
--			FolioLicencia = @FolioLicencia,
--			Oficina = @Oficina,
--			RutEjecutivo = @RUT_EJECUTIVO,
...
--			SELECT @OFC_EX = y.Oficina, @FEC_EX = CONVERT(VARCHAR(20),CONVERT(DATE,x.FechaIngreso))
--			FROM licencias.TabLic_IngresoLicencia x
--			INNER JOIN dbo.TabMotor_Oficinas y ON x.Oficina = y.Cod_Oficina
--			WHERE x.RutAfiliado =@RutAfiliado
--			AND x.FolioLicencia = @FolioLicencia

--			DECLARE @ERRMSG VARCHAR(200)= 'Esta Licencia ya existe en Motor. Fecha de Ingreso: ' + @FEC_EX + ', Oficina de Ingreso: ' + @OFC_EX
...
--				INSERT INTO TabLic_IngresoLicencia
--				(
--					RutAfiliado,
--					NombreAfiliado,
--					FolioLicencia,
