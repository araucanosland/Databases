IF EXISTS (SELECT 1 FROM sys.objects WHERE object_id = OBJECT_ID('[mae].[spMotor_BuscarAfiliadoScan]') AND type = 'P')
DROP PROCEDURE [mae].[spMotor_BuscarAfiliadoScan]
GO
CREATE procedure [mae].[spMotor_BuscarAfiliadoScan]
(
	@RutAfiliado int
)
as
...
SELECT distinct 
     ...
     ,A.PeriodoUltimaRenta
	  ...
	  ,isnull(MotivoNM.Motivo,'') MotivoNM
...
left join (SELECT Motivo, RutAfiliado FROM TabMotor_No_Molestar
							WHERE Id_NoMolestar = (SELECT MAX(Id_NoMolestar)
							FROM TabMotor_No_Molestar where LEFT(RutAfiliado,CHARINDEX('-',RutAfiliado)-1) = @RutAfiliado
							GROUP BY RutAfiliado) and LEFT(RutAfiliado,CHARINDEX('-',RutAfiliado)-1) = @RutAfiliado and FlagEstado = 1 ) MotivoNM
							on convert(varchar,A.RutAfiliado)+'-'+A.DvAfiliado = MotivoNM.RutAfiliado							
